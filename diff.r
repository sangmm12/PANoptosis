...
my_data = read.csv('CIBER.csv',header=T,check.names=F)
...
pdf(paste("CIBER_",file_name,"/",name,".pdf",sep=''),width=temp_length,height = 8)

p <- ggboxplot(dat, x = "Gene", y = "value",
               palette = "npg",
               color = "Group",
               x.text.angle=60,x.text.size=20,shape = "rx",adjust=2,notch=T)
...
dev.off()
#rm(list=c('dat','other_file','exp_file','all_name','xx','p_value','p','gene_list','temp_length'))
}
}
